...
static int virtnet_poll_tx(struct napi_struct *napi, int budget)
{
	...
	if (free_old_xmit_skbs(txq, sq, budget) < budget) {
		complete = true;
...
static netdev_tx_t start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (!use_napi) {
		if (used_napi) {
			napi_disable_pending(&sq->napi);
			net_info_ratelimited("%s: extra napi disable\n", __FUNCTION__);
		}

		/* Free up any pending old buffers before queueing new ones. */
		free_old_xmit_skbs(txq, sq, virtqueue_get_vring_size(sq->vq));
	} else {
		/* TODO: find a cleaner solution to bootstrap napi */
		if (!used_napi) {
			net_info_ratelimited("%s: extra napi sched\n", __FUNCTION__);
